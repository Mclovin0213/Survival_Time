...
func show_crafting_info(item_key : ItemConfig.Keys) -> void:
	...
	var blueprint := ItemConfig.get_crafting_blueprint(item_key)
...
func update_inventory(inventory : Array) -> void:
	...
	for crafting_button in crafting_button_container.get_children():
		var costs := ItemConfig.get_crafting_blueprint(crafting_button.item_key).costs
...
func crafting_button_pressed(item_key : ItemConfig.Keys) -> void:
	EventSystem.INV_delete_crafting_blueprint_costs.emit(ItemConfig.get_crafting_blueprint(item_key).costs)
